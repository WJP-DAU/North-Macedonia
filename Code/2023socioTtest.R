...
ttest_dem_gpp.fn <- function(data.df = data_2023.df, 
                             section = "govSupp"){
...
openxlsx::write.xlsx(diffmeans.list, file = "Outputs/2023ttest_differences.xlsx")

# Read the data 
df_2017 <- read_excel("Outputs/2017ttest_differences.xlsx")
df_2023 <- read_excel("Outputs/2023ttest_differences.xlsx")

# Remove the country column 
df_2017 <- select(df_2017, -country)
df_2023 <- select(df_2023, -country)

# Not to be duplicated columns
no_duplicate_columns <- c('label', 'variable')

# Rename the columns 
colnames(df_2017) <- ifelse(colnames(df_2017) %in% no_duplicate_columns, colnames(df_2017), paste0('2017_', colnames(df_2017)))

colnames(df_2023) <- ifelse(colnames(df_2023) %in% no_duplicate_columns, colnames(df_2023), paste0('2023_', colnames(df_2023)))

# Combine the two dataframes
df_combined <- bind_cols(df_2017, select(df_2023, -all_of(no_duplicate_columns)))

# Create the difference column 
df_combined$difference <- df_combined$`2023_diff` - df_combined$`2017_diff`

# Create the direction column 
df_combined$direction <- case_when(
  df_combined$difference > 0 ~ "positive",
  df_combined$difference < 0 ~ "negative",
  TRUE ~ "none" 
)

# combined dataframe to excel
openxlsx::write.xlsx(df_combined, file = "Outputs/merged_ttest_differences.xlsx")
